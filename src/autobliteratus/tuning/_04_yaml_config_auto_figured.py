"""Tuning type 4: yaml config auto figured.
...
class AblationPlan:
    ...
    direction_count: int
    component_split: dict = field(default_factory=dict)
...
class YamlConfigAutoFigurer:
    ...
    def __init__(self, model_path: str, work_dir: str = "./abliterate_workspace") -> None:
    ...
    def _classify(self, cfg: dict) -> tuple[str, str]:
    ...
    def _layer_plans(self, refusal_probe: dict) -> list[LayerPlan]:
        norms = refusal_probe.get("refusal_norms") or []
        return [
            LayerPlan(
                index=i,
                refusal_direction_norm=float(n),
